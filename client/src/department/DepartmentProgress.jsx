...
const DepartmentProgress = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:3000/api/department-progress', {
          headers: {
    ...
    <div className='grid-container'>
      ...
      <div className="department-progress">
        ...
        <div className="export-btn">
          <button onClick={exportToPDF} className="btn btn-primary">Export to PDF</button>
        </div>
        <table id="department-table" className="table table-striped table-bordered">
          <thead>
            <tr>
              <th>Name</th>
              ...
              <th onClick={toggleSortDirection} style={{ cursor: 'pointer' }}>
                Upload Count {sortDirection === 'asc' ? <FaSortUp /> : <FaSortDown />}
              </th>
            </tr>
          </thead>
          <tbody>
