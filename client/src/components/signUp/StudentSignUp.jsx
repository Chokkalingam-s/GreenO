...
const StudentSignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (token) {
        await axios.post('http://localhost:3000/send-otp', { email: formData.email });
        setOtpSent(true);
  ...
  const verifyOtp = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:3000/verify-otp', { email: formData.email, otp });
              ...
              <>
                ...
                <div className="form-group mb-3">
                  ...
                  {errors.collegeRegisterNumber && (
                    <small className="text-danger">{errors.collegeRegisterNumber}</small>
                  )}
                </div>
